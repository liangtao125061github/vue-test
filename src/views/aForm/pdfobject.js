...
const isSafariIOSDesktopMode = (nav.platform !== undefined &&
...
const isFirefoxWithPDFJS = (!isMobileDevice && /irefox/.test(ua)) ? (parseInt(ua.split('rv:')[1].split('.')[0], 10) > 18) : false
...
const generatePDFJSMarkup = function(targetNode, url, pdfOpenFragment, PDFJS_URL, id, omitInlineStyles) {
...
const generatePDFObjectMarkup = function(embedType, targetNode, targetSelector, url, pdfOpenFragment, width, height, id, omitInlineStyles) {
  // Ensure target element is empty first
  emptyNodeContents(targetNode)
...
const embed = function(url, targetSelector, options) {
  ...
  const assumptionMode = (typeof opt.assumptionMode === 'boolean') ? opt.assumptionMode : true
  const forcePDFJS = (typeof opt.forcePDFJS === 'boolean') ? opt.forcePDFJS : false
  const supportRedirect = (typeof opt.supportRedirect === 'boolean') ? opt.supportRedirect : false
  const omitInlineStyles = (typeof opt.omitInlineStyles === 'boolean') ? opt.omitInlineStyles : false
  const suppressConsole = (typeof opt.suppressConsole === 'boolean') ? opt.suppressConsole : false
  const forceIframe = (typeof opt.forceIframe === 'boolean') ? opt.forceIframe : false
  ...
  if (supportsPDFs || (assumptionMode && isModernBrowser && !isMobileDevice)) {
    ...
    const embedtype = (forceIframe || (supportRedirect && isSafariDesktop)) ? 'iframe' : 'embed'
